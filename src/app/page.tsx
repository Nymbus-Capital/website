          ...
          <AnimatePresence mode="wait">
            <motion.div key={current} initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -60 }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="aspect-[5/3] rounded-xl overflow-hidden bg-slate-100"><img src={newsItems[current].image} alt={newsItems[current].title} className="w-full h-full object-cover" /></div>
                <div className="flex flex-col justify-center py-4">
                  ...
                  <div className="mt-6">
                    <button onClick={() => setOpenArticle(current)} className="text-blue-600 font-medium text-sm inline-flex items-center gap-1.5 hover:gap-2.5 transition-all cursor-pointer">{t('home.news.readMore')} <ArrowRight className="w-4 h-4" /></button>
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        {openArticle !== null && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setOpenArticle(null)}>
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} className="bg-white rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto relative" onClick={(e) => e.stopPropagation()}>
              <div className="aspect-[5/3] w-full overflow-hidden rounded-t-2xl"><img src={newsItems[openArticle].image} alt={newsItems[openArticle].title} className="w-full h-full object-cover" /></div>
              <button onClick={() => setOpenArticle(null)} className="absolute top-4 right-4 w-8 h-8 rounded-full bg-white/90 shadow-md flex items-center justify-center hover:bg-white transition-colors"><span className="text-slate-600 text-lg leading-none">&times;</span></button>
              <div className="p-8">
                <div className="flex items-center gap-3 mb-4">
                ...
                {newsItems[openArticle].article.split('\n\n').map((paragraph, i) => (<p key={i} className="text-slate-600 leading-relaxed mb-3">{paragraph}</p>))}
    ...
    <main className="bg-white">
      <section className="min-h-[90vh] flex flex-col items-center justify-center px-6 py-20 relative overflow-hidden">
        ...
        <div className="max-w-3xl text-center relative z-10">
          ...
          <motion.p className="text-xl text-slate-600 mb-10 leading-relaxed max-w-2xl mx-auto" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.3 }}>{t('home.hero.subtitle')}</motion.p>
